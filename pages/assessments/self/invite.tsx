...
const leaderTypes: Record<string, { name: string; description: string }> = {
  'low-low': { name: 'Overlooked', description: 'You may not always be seen or heard in your organization...' },
  'medium-low': { name: 'Functional Contributor', description: 'You show up reliably and contribute steadily...' },
  'high-low': { name: 'Hidden Leader', description: 'You lead from within — with integrity and quiet strength...' },
  'low-medium': { name: 'Underutilized', description: 'You bring energy, ambition, and capability...' },
  'medium-medium': { name: 'Developing Leader', description: 'You’re finding your leadership footing...' },
  'high-medium': { name: 'Promotable Performer', description: 'You deliver consistently and are trusted...' },
  'low-high': { name: 'Polished Performer', description: 'You likely make a strong impression...' },
  'medium-high': { name: 'Emerging Leader', description: 'You’re stepping into leadership with curiosity...' },
  'high-high': { name: 'Visible Leader', description: 'You bring both presence and principle...' }
};

export default function InviteSelfAssessment() {
    ...
    if (token) {
      supabase
        ...
        .then(({ data, error }) => {
          ...
          else console.error('Token not valid');
        });
    }
  }, [token]);
  ...
  if (!user) return <div className="p-6 text-center">Loading or invalid link...</div>;
